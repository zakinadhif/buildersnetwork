...
- **App shell with a persistent left sidebar**, replacing standalone route-pages. Its live destinations match the current mockup rail: **Scroll** (the home feed), **Karya** (catalogue), and **People** (builder directory). `Cari Kolaborator`, `Minat Saya`, and `Karya Saya` are retired from this rail; Matchmaking stays a later milestone. The **AI assistant** remains reachable as an opt-in tool, but is not a fourth primary destination.
...
- **The Launchpad closure bar includes the primary funnels.** It covers the literal entry path (`/welcome` → `/verify-email` → `/mulai` → Scroll), the persistent three-destination shell, the Scroll surface, and the core transitions from Scroll/Karya into karya detail and creation plus Scroll/People into member profiles. `/karya/:id`, `/karya/new` (including the existing manual/AI choice), and `/member/:id` must be functional, grounded in their current mockups, built from the shared design system, and polished as hero edges rather than merely reachable. The standalone onboarding/review/matches flow remains opt-in and is not a closure gate.
...
A new user signs up and lands on Scroll inside the persistent `Scroll` / `Karya` / `People` sidebar shell **without being forced through the AI chat**; the AI assistant is reachable as an opt-in tool; Scroll renders the reverse-chronological karya-update feed; and the primary paths into `/karya/:id`, `/karya/new`, and `/member/:id` are functional, mockup-grounded, use the shared design system, and cover their meaningful loading, empty, error, and success states.
