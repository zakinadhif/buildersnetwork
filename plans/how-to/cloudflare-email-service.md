...
> [Resend](https://resend.com) when a `RESEND_API_KEY` secret is set, and falls
> back to a no-op provider when neither is configured (see `selectEmail` in
> `apps/api/src/lib/email.ts`). This doc covers the Cloudflare-native path; for
> the Resend path see `libs/email/src/resend.ts` and `DEPLOY_CLOUDFLARE.md`.
