...
- `seekingType` is a closed vocabulary validated in code, not a pg enum (repo convention; see [retro](../archive/retro.txt)).
- **Presentation stays minimal — exactly what FR-5/27/28 call for**: an intent badge on the profile + a filterable seeker board. The [Cari Kolaborator mockups](../../apps/mockups/src/) are design *reference* for the UI tasks, not a blocking decision — their two-sided ambition belongs to [Karya openings](karya-openings.md).
