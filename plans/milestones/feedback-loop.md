...
- **Surface treatment** — needs a `[Desain]` before the `[Fitur]` is groomed (Gate B): how a comment thread renders on the karya timeline, and what (if anything) the feed teaser shows about it.
