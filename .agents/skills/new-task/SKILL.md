...
3. For buildable work, author the cold-completable contract in this order: `## Kenapa`, `## Ruang lingkup`, `## Batas (touch / don't touch)`, `## Kriteria terima`, optional `## Dependensi`, and `## Di luar lingkup`. Include relevant treatment/milestone and required Vitest or Playwright validation. Split by vertical deliverable, not DB/API/UI layers. For `[Diskusi]`, use the canonical structure in `$open-discussion`; do not force task acceptance criteria onto an undecided question.
