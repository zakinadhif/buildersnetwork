...
Keep the proposal's **intended-features list in the doc PR**, never as a markdown task list in the proposal's issue body. A body task list renders its *own* "X of Y checkboxes" bar, separate from the sub-issue bar — GitHub does not merge the two, so you'd get two competing progress indicators tracking different things (checked boxes vs. shipped issues). One bar, driven by real sub-issues, is the honest signal.
